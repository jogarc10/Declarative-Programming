...
--Exer 12
    ...
    --last(take (10^7) [1..10^20]) -> medio
    --head(take (10^7) ([1..100]++[1..10^20])) -> poco
    --last(take 100 ([1..10^20]++[1..100])) -> poco
    --last(drop 100 ([1..10^20]++[1..100])) -> mucho
    --head(drop (10^7) ([1..10^20]++[1..100])) -> medio
    --[1..10^7] == [1..10^7] -> medio
    --[1..10^20] == [1..10^20] -> mucho
    --[1..10^20] == [1..10^20+1] -> mucho
    --[1..10^20] == [2..10^20] -> poco
    --head(reverse [1..10^7]) -> medio
    --last(reverse [1..10^7]) -> medio
    --reverse [1..10^20] == reverse [1..10^20+1] -> mucho


--Exer 13

primo_2 n = let x = divisors n in length(x) == 1 && (x == [1])

first_gt_1000p = first_prime [1000..]

first_prime (x:xs)
  | primo_2 x = x
  | otherwise = first_prime(xs)

first_100_prime = primes 0 [1..]

primes n (x:xs)
  | n == 100 = []
  | primo_2 x = x:(primes(n+1) xs)
  | otherwise = primes n xs
